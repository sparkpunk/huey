<template>
  <div class="fixed z-50 inset-0 flex flex-col items-center justify-center p-8">
    <div class="absolute inset-0 bg-gray-900 opacity-90"></div>
    <div class="relative min-w-full bg-white rounded-xl shadow-xl overflow-hidden"
      :style="{height: height}">
      ...
      <div class="absolute top-0 left-0 right-0 flex items-center w-full h-16 px-4 border-b border-gray-100">
        ...
        <input class="appearance-none w-4 h-4 border border-gray-500 checked:bg-blue-500 checked:border-blue-500 focus:outline-none rounded" id="showDetails" type="checkbox" @change="showDetails = !showDetails" />
        ...
        <table class="border-collapse">
          <thead class="bg-white border-b border-gray-100">
            <tr class="bg-white">
              ...
              <th class="font-normal px-6 text-2xs text-gray-600 bg-gray-50" v-for="col in data.slice().reverse()" :key="col.contrast">
                {{ col.color }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="bg in data" :key="bg"
              :style="{background: bg.background}">
              <td class="h-16 p-2 text-2xs text-right text-gray-600 bg-gray-50 border-r border-gray-100">
                {{ bg.background }}
              </td>
              <td v-for="text in data.slice().reverse()" :key="text"
